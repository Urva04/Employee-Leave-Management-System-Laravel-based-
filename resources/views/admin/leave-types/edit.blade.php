                ...
                <form method="POST" action="{{ route('admin.leave-types.update', $leaveType) }}">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code', $leaveType->code) }}" maxlength="10" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="days_per_year" class="form-control @error('days_per_year') is-invalid @enderror" value="{{ old('days_per_year', $leaveType->days_per_year) }}" min="0" max="365" required>
                            @error('days_per_year') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-4">
                            <div class="form-check mt-4">
                                <input type="checkbox" name="is_paid" value="1" class="form-check-input" id="isPaid" {{ old('is_paid', $leaveType->is_paid) ? 'checked' : '' }}>
                                <label class="form-check-label" for="isPaid">Paid Leave</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check mt-4">
                                <input type="checkbox" name="requires_attachment" value="1" class="form-check-input" id="reqAttach" {{ old('requires_attachment', $leaveType->requires_attachment) ? 'checked' : '' }}>
                                <label class="form-check-label" for="reqAttach">Requires Attachment</label>
                            </div>
                        </div>
                    </div>
                    <div class="mt-4 d-flex gap-2">
                        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i>Update Leave Type</button>
